...
import { Client, ClientGrpc, GrpcService, Transport } from '@nestjs/microservices';
...
interface OrderGrpcClient {
  ...
  findAllOrders(
    ...
    metadata?: Metadata): Observable<{ orders: Order[] }>;
  findOneOrder(
    data: { order_id: string },
    metadata?: Metadata): Observable<{ order: Order }>;
}
...
export class OrdersService implements OnModuleInit {
  ...
  async findAll(account_id: string) {
    const metadata = new Metadata();
    metadata.set("authorization", "Bearer 1234");
    const result = await lastValueFrom(this.orderGrpcClient.findAllOrders({ account_id }, metadata));
  ...
  async findOne(id: string) {
    const metadata = new Metadata();
    metadata.set("authorization", "Bearer 1234");
    const result = await lastValueFrom(this.orderGrpcClient.findOneOrder({ order_id: id }, metadata));
    return result.order;
  }
